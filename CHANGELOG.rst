...
Unreleased
----------
- CLI: ``oo docs serve [--dir D] [--port N] [--host H] [--index PAGE] [--rebuild CMD]`` serves a documentation tree locally with a review layer: every served HTML page gets the ✎ commenter (``_commenter.py``; comments POST to ``/comment`` and land one JSON document per line in ``<root>/__notes__/site-comments.jsonl``, read and written by pathlike's JSONL engine; ``/comments?page=`` lists the open ones, ``/comment-edit`` and ``/comment-delete`` change them; a malformed comments file is reported with its line, never rewritten), and an image dropped on a page is written under ``<root>/images/`` (validated segments, image extensions only, never outside the root). ``/`` falls back to the first of ``site/``, ``docs/``, ``build/html/``, ``docs/_build/html/`` with an ``index.html``. Binds all interfaces by default (``$PYGIM_HOST`` or ``--host 127.0.0.1`` for localhost); ``--rebuild`` runs a shell command in the served directory first, aborts on failure, and reports the pages it added and removed (a ``PathSet`` difference). The server owns a ``PathStore``: the root, the comments file, every request's path and the ``GET /pages`` inventory are rows of that one table (the module's default store never sees request traffic), and ``?page=`` is compared as a path so spellings collapse. A ``.md`` page is rendered to HTML on the fly (the ``markdown`` package; Mermaid fences become live diagrams) with the same commenter, ``README.md`` / ``index.md`` / ``00_overview.md`` stand in for a missing ``index.html``, and the inventory counts Markdown pages. ``oo`` itself is now a group: a verb runs it, free text goes to the assistant, nothing shows the help.
Changed
~~~~~~~
