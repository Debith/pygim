...
- Persistence: ``ConnectionString`` value object (``core/connection_string.h``, exposed to Python) consolidates connection-string handling — a raw DSN or ``mssql+pyodbc://`` URL parses via a builder/abstract-factory pair in the pybind-free core. ``str()``/``repr()``/``render()`` mask the password by default (``render(reveal=True)`` for the connect string), so credentials no longer leak into logs or tracebacks. The URL→DSN translation moved out of Python into the core, so every entry point (top-level ``pygim.acquire_datastore`` included) shares it, and the token-auth conflict check now runs on the parsed keys — catching userinfo in a URL (``user:pass@``), which the old raw-substring scan missed. The connection pool and connection now hold the value object (rendering to a DSN only at the connect boundary and masking it in logs); ``PacketSize`` is applied structurally via ``ConnectionString::with_packet_size`` (retiring the bespoke ``ensure_packet_size``), and the access-token byte-packing moved to ``core::AccessTokenPacker``.
...
- Persistence: An unsupported connection-URL scheme (e.g. ``postgresql://``) now raises ``GimError`` (from the C++ ``ConnectionString`` parser) instead of ``ValueError``; the message still says "Unsupported URL scheme". The internal ``_translate_conn_str`` helper is retained as a thin shim over ``ConnectionString``.
