...
Changed
~~~~~~~
- Release: a release is a branch, ``release/<version>``, cut from ``main``. ``.github/workflows/release.yml`` builds wheels for Linux x86_64, macOS arm64 and Windows AMD64 on CPython 3.9-3.14 and runs the whole test suite against each installed wheel. Then it publishes to PyPI through trusted publishing, tags ``v<version>`` and creates the GitHub release. Start one with ``gh workflow run release.yml -f bump=minor`` or by pushing ``release/<version>``; see ``docs/releasing.md``. It replaces ``publish.yml`` and ``auto-tag.yml``: tags pushed by ``auto-tag.yml`` could never trigger ``publish.yml``, so no release had reached PyPI.
