...
Unreleased
----------
- CLI: ``oo docs serve [--dir D] [--port N] [--host H] [--index PAGE] [--rebuild CMD]`` serves a documentation tree locally with a review layer: every served HTML page gets the ✎ commenter (``_commenter.py``; comments POST to ``/comment`` and land one JSON document per line in ``<root>/__notes__/site-comments.jsonl``, read and written by pathlike's JSONL engine; ``/comments?page=`` lists the open ones, ``/comment-edit`` and ``/comment-delete`` change them; a malformed comments file is reported with its line, never rewritten), and an image dropped on a page is written under ``<root>/images/`` (validated segments, image extensions only, never outside the root). ``/`` falls back to the first of ``site/``, ``docs/``, ``build/html/``, ``docs/_build/html/`` with an ``index.html``. Binds all interfaces by default (``$PYGIM_HOST`` or ``--host 127.0.0.1`` for localhost); ``--rebuild`` runs a shell command in the served directory first, aborts on failure, and reports the pages it added and removed (a ``PathSet`` difference). The server owns a ``PathStore``: the root, the comments file, every request's path and the ``GET /pages`` inventory are rows of that one table (the module's default store never sees request traffic), and ``?page=`` is compared as a path so spellings collapse. Opening a ``.md`` page GENERATES ``<stem>.html`` beside it (the ``markdown`` package; Mermaid fences become live diagrams; ``.md`` links are rewritten to their ``.html``; a marker names the source; regenerated when the Markdown is newer; a hand-written page is never overwritten) and redirects there, so the commenter works on the HTML page and comments key on it; ``README.md`` / ``index.md`` / ``00_overview.md`` stand in for a missing ``index.html``, ``/pages`` lists HTML pages plus Markdown not yet generated, and every Markdown page's HTML is generated at startup so no first open renders. ``oo`` itself is now a group: a verb runs it, free text goes to the assistant, nothing shows the help.
Changed
~~~~~~~
- Release: the distribution is ``pygim`` on PyPI, the project the name was registered as (``pyproject.toml`` said ``python-gimmicks``, which was never published): ``pip install pygim``.
- Release: nothing is released from ``main``; a release runs on its branch, ``release/<version>``. ``.github/workflows/release.yml`` builds wheels for Linux x86_64, macOS arm64 and Windows AMD64 on CPython 3.9-3.14 and runs the whole test suite against each installed wheel. Then it publishes to PyPI through trusted publishing, tags ``v<version>`` and creates the GitHub release. Run on ``main`` (``gh workflow run release.yml -f bump=minor``), it cuts ``release/<version>`` and starts the release there; pushing a ``release/<version>`` branch releases it too. See ``docs/releasing.md``. It replaces ``publish.yml`` and ``auto-tag.yml``: tags pushed by ``auto-tag.yml`` could never trigger ``publish.yml``, so no release had reached PyPI.
- Build (BREAKING for mixed pyarrow installs): the wheel's pyarrow requirement is pinned to the ``major.minor`` release it was built against (``pyarrow==23.0.*`` instead of ``pyarrow>=15``; ``setup.py::_pin_arrow_abi``). The Arrow-linked extensions load ``libarrow.so.<major><minor>`` and fail to import under any other pyarrow. ``[project] dependencies`` is now dynamic; the list lives in ``[tool.pygim.build]``.
- PathLike: The path VALUE is now an RFC 3986 ``uri`` (``uri.h``: constexpr parse/render/normalise) behind a pathlib-shaped API, with the platform rules as constexpr strategy policies (``posix_strategy``/``windows_strategy``, ``basic_file<Strategy>``) — the whole path algebra (name/stem/suffix/parts/parent/join/with_*) is constexpr and proven at compile time against pathlib for BOTH strategies on every host (``tests/static/pathlike_parity_proofs.cpp``, generated from pathlib by ``gen_pathlike_parity_proofs.py``). Visible changes: ``str()``/``os.fspath()`` return pathlib's normalised spelling (``a//b/`` -> ``a/b``, ``./x`` -> ``x``, ``''`` -> ``.``); ``.uri`` renders an absolute path per RFC 3986 with percent-encoding (``file:///a%20b``, ``file://host/share/x``; relative paths keep ``file://<path>``); ``pygim.path()`` accepts ``file://`` URIs as input (decoded like ``pathlib.Path.from_uri``; a remote host is a UNC path on Windows and a ValueError on POSIX; relative file URIs and other schemes raise ValueError); ``with_name``/``with_suffix`` validate their arguments like pathlib (ValueError).
- PathLike/perf: ``pygim.path`` and every derived-path operation got cheaper without changing behaviour — str/bytes/PathLike arguments are converted directly (no ``fs::path`` round trip), the typed engine subclass is moved into rather than copied, ``__hash__`` hashes the value (FNV-1a over what ``__eq__`` compares) instead of rendering and hashing the path text, and a ``file://`` URI is parsed once instead of twice. Measured on 200k paths (min of 5): construction -22%, hashing/set -32%, URI input -46%, ``.uri`` -17%. The package's lazy top-level exports (``pygim.path`` etc.) are now cached in the module after the first access (a PEP 562 miss re-imported and re-fetched the name on EVERY access: 720 ns -> 24 ns per ``pygim.path`` lookup).
- Utils/memory: ``pygim.utils.rss_bytes()`` / ``rss_mb()`` / ``peak_rss_bytes()`` / ``peak_rss_mb()`` — the process's resident memory as one syscall on Linux, macOS and Windows (``utils/memory.h``; ~1 us on Linux via a procfs pread, the descriptor opened once). Process-wide numbers for a benchmark's before/after delta, never a per-object size: every component reports its own exact ``bytes`` — ``PathSet.stats()`` and ``PathStore.stats()`` now carry that total under the same key. ``benchmarks/_bench.py`` uses the probe.
- PathLike (BREAKING): ``pygim.path`` is ONE class — a handle (table, row, pin) on a ``PathStore``'s table where every distinct path component is stored once (``path_table.h`` over the mapping toolkit's interner and trie). ``file`` and ``fileview`` are gone; the typed classes are ``yamlpath`` / ``jsonpath`` / ``tomlpath`` / ``jsonlpath``, and ``path("x.yaml")`` is a ``yamlpath`` through the classes' shared metaclass. The constructor interns; ``store=`` chooses the table once and every derived path inherits it (``default_store()`` is the module's), so a program scopes a lifetime by argument. Equal paths compare and hash alike whatever their spelling; two handles are not promised to be the same object. name/parent/suffix are table reads; the core value is built on demand for ``with_*``, URIs and filesystem calls. Measured: ``path(s)`` ~430 ns against ``PurePath(s)`` ~380 (which parses nothing), ``p.parent`` 300 against 890; ~140 bytes per handle plus ~125 of table per path. Strategies gained ``tokenise()`` (a streaming parse; the POSIX one is allocation-free and proven equal to ``parse_into``).
- PathLike/PathSet (``pygim.PathSet``): many paths as ONE table — rows of a ``PathStore``'s table with a member bitmap, ~100 bytes per path. Iteration yields path objects over the table; ``scan()`` reuses one; ``filter_suffix``/``filter_name``/``filter_absolute`` and ``|``/``&``/``-`` work on rows (~1-10 ns per path on a shared table; across tables the larger table is copied and the other set's rows mapped in); ``count_union`` / ``count_intersection`` / ``count_difference`` answer the size of an algebra result as a popcount without building it; ``reserve(n)`` sizes the table up front (automatic for sized iterables); ``to_list()`` renders the members (an Arrow C Data Interface export was tried and dropped — ``docs/design/pathset_storage.md``). Benchmark: ``benchmarks/pathset_prototype.py``.
- PathLike/PathSet: the former ``pygim.pathset`` extension is FOLDED into ``pygim.pathlike.PathSet`` and removed (BREAKING for ``from pygim.pathset import ...``; ``pygim.PathSet`` now names the pathlike class). What carried over, now as rows of one table: ``PathSet(one_path)`` and ``PathSet(iterable)``, ``bool``, ``==``, ``in``, ``+``/``-`` with a PathSet or path(s), ``clone()``, ``cwd()``, ``read_all_files()``, ``match_pattern()``, and the filter algebra — ``Filter`` (``ext()``, ``name()``, ``absolute()``; ``&``, ``|``, ``~``) with the lazy ``Query`` (``ps & f``; ``eval()``, ``len()``, iteration) plus ``ps.filter(f)``. Differences: iteration yields path objects rather than ``pathlib.Path``; sets are values, so ``ps -= x`` rebinds rather than edits; filters read the table, never the filesystem; ``match_pattern("", "")`` is True (pathlib's rule) where the old matcher said False. ``p.pathset(pattern)`` returns a PathSet over p's own table.
- Core/mapping: the tables PathSet and path objects are built on are toolkit components, pybind-free and constexpr, with their laws proven by every build (``tests/static/mapping_proofs.cpp``): the ``interner`` concept with ``basic_flat_interner<Id>`` / ``basic_hashed_interner<Id>`` engines (``mapping/intern.h`` — a sibling of ``storage``, not an engine of it: it owns the bytes, assigns the id, and never erases), the hash-consed ``basic_trie<RowId, Key>`` with a memoised ``row_map`` (``mapping/trie.h``), both proven over 32- and 16-bit ids, the dense ``basic_id_set<Id, Word>`` with ``id_set`` as its default (``mapping/id_set.h``; proven over 16/32/64-bit ids and 32/64-bit words; ``PathSet.count_union`` / ``count_intersection`` / ``count_difference`` answer the size of an algebra result as a popcount over the bitmaps without building it — 20-90x the build at 1M paths, with ``-mpopcnt`` passed to the extensions that compile the toolkit), one hashing header (``utils/hash.h``; the wiring adapters' key hashes use its ``combine``). ``path_table`` is ``basic_path_table<Interner>`` over them, and its own laws (value, hash, render, parent agree with ``file``) are proven over the flat interner in constant evaluation. Design: ``docs/design/mapping_toolkit.md``.
- PathLike: The engine registry is now OPEN. Every engine is one header in ``src/_pygim_fast/pathlike/adapter/engines/`` (a struct with a constexpr ``engine_info`` plus static ``load``/``write``); the build discovers the directory into a generated type list (``[extension.typelist]`` in ``ext.pathlike.toml``, ``setup.py::_apply_typelist``), and extension/name lookup, error inventories, the ``<name>path`` typed classes, docstrings and the new ``pathlike.ENGINES`` record are all folds over it. ``enum class Engine`` and the hand-maintained tables are gone (the extension and selector tables are ``StaticRegistryCore`` instances built from the pack); the compile-time proofs are generic over the pack (positive and negative, ``tests/static/pathlike_core_proofs.cpp``) and asserted on the real pack in every build, with a C++26 ``static_assert`` message naming the offending engine where the compiler supports it. With P2996 reflection (GCC 16, ``-freflection``, passed via the new ``flags_if_supported`` manifest key when the compiler accepts it) the build additionally proves that the generated list equals the engine structs the compiler sees and that every engine is named after its struct. Adding a format is adding a file. See ``docs/design/pathlike_engine_registry.md``.
