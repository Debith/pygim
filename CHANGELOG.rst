...
- Persistence: ``ConnectionString`` value object (``core/connection_string.h``, exposed to Python) consolidates connection-string handling — a raw DSN or ``mssql+pyodbc://`` URL parses via a builder/abstract-factory pair in the pybind-free core. ``str()``/``repr()``/``render()`` mask the password by default (``render(reveal=True)`` for the connect string), so credentials no longer leak into logs or tracebacks. The URL→DSN translation moved out of Python into the core, so every entry point (top-level ``pygim.acquire_datastore`` included) shares it, and the token-auth conflict check now runs on the parsed keys — catching userinfo in a URL (``user:pass@``), which the old raw-substring scan missed.
