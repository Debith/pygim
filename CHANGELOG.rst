...
- Core/mapping: the tables PathSet and PathStore were built on are toolkit components now, pybind-free and constexpr, with their laws proven by every build (``tests/static/mapping_proofs.cpp``): the ``interner`` concept with ``flat_interner`` / ``hashed_interner`` engines (``mapping/intern.h`` — a sibling of ``storage``, not an engine of it: it owns the bytes, assigns the id, and never erases), the hash-consed ``trie`` with a memoised ``row_map`` (``mapping/trie.h``), the dense ``id_set`` (``mapping/id_set.h``), one hashing header (``utils/hash.h``; the wiring adapters' key hashes use its ``combine``), the flyweight ``token`` (``utils/flyweight.h``) and the two adapter utilities behind the store: ``weak_slots<T>`` (``utils/flyweight_adapter.h``) and ``ambient<T>`` with its bound ``use_<x>()`` scope (``utils/ambient_adapter.h``). ``path_table`` is ``basic_path_table<Interner>`` over them, and its own laws (value, hash, render, parent agree with ``file``) are proven over the flat interner in constant evaluation. Design: ``docs/design/mapping_toolkit.md``.
