...
- Core/mapping: the tables PathSet and PathStore were built on are toolkit components now, pybind-free and constexpr, with their laws proven by every build (``tests/static/mapping_proofs.cpp``): the ``interner`` concept with ``flat_interner`` / ``hashed_interner`` engines (``mapping/intern.h`` — a sibling of ``storage``, not an engine of it: it owns the bytes, assigns the id, and never erases), the hash-consed ``trie`` with a memoised ``row_map`` (``mapping/trie.h``), the dense ``basic_id_set<Id, Word>`` with ``id_set`` as its default (``mapping/id_set.h``; proven over 16/32/64-bit ids and 32/64-bit words; ``PathSet.count_union`` / ``count_intersection`` / ``count_difference`` answer the size of an algebra result as a popcount over the bitmaps without building it — 20-90x the build at 1M paths, with ``-mpopcnt`` passed to the extensions that compile the toolkit), one hashing header (``utils/hash.h``; the wiring adapters' key hashes use its ``combine``), the flyweight ``token`` (``utils/flyweight.h``) and the two adapter utilities behind the store: ``weak_slots<T>`` (``utils/flyweight_adapter.h``) and ``ambient<T>`` with its bound ``use_<x>()`` scope (``utils/ambient_adapter.h``). ``path_table`` is ``basic_path_table<Interner>`` over them, and its own laws (value, hash, render, parent agree with ``file``) are proven over the flat interner in constant evaluation. Design: ``docs/design/mapping_toolkit.md``.
- PathLike (BREAKING): ONE class, ``pygim.path`` — a handle (table, row, pin) on a PathStore's table; ``file``, ``fileview``, ``path()``-the-factory, ``store()`` and ``use_store()`` are gone, and the typed classes are ``yamlpath`` / ``jsonpath`` / ``tomlpath`` / ``jsonlpath``. The constructor interns (``path("x.yaml")`` is a ``yamlpath`` through the classes' metaclass); ``store=`` chooses the table once and every derived path inherits it, so a program scopes a lifetime by argument (``default_store()`` is the module's). Equal paths compare and hash alike whatever their spelling; two handles are NOT promised to be the same object (as CPython interns strings without promising ``is``). A handle is ~70 bytes instead of ~540; name/parent/suffix are table reads; the core value is built on demand for ``with_*``, URIs and filesystem calls. Design: ``docs/design/pathlike_flyweight.md`` (superseded section).
- PathLike/PathSet: the former ``pygim.pathset`` extension is FOLDED into ``pygim.pathlike.PathSet`` and removed (BREAKING for ``from pygim.pathset import ...``; ``pygim.PathSet`` now names the pathlike class). What carried over, now as rows of one table: ``PathSet(one_path)`` and ``PathSet(iterable)``, ``bool``, ``==``, ``in``, ``+``/``-`` with a PathSet or path(s), ``clone()``, ``cwd()``, ``read_all_files()``, ``match_pattern()``, and the filter algebra — ``Filter`` (``ext()``, ``name()``, ``absolute()``; ``&``, ``|``, ``~``) with the lazy ``Query`` (``ps & f``; ``eval()``, ``len()``, iteration) plus ``ps.filter(f)``. Differences: iteration yields ``fileview`` (an os.PathLike with name/suffix/parent) rather than ``pathlib.Path``; sets are values, so ``ps -= x`` rebinds rather than edits; filters read the table, never the filesystem; ``match_pattern("", "")`` is True (pathlib's rule) where the old matcher said False. ``file.pathset(pattern)`` returns a PathSet over the current store's table, so its views and ``path()`` objects meet without re-interning.
- PathLike/PathStore: ``pygim.path()`` is now a flyweight — equal paths made through ``path()`` and derived-path operations are the SAME object while anything holds them (``file(...)`` stays the raw constructor; ``engine=`` pins bypass). ``pathlike.PathStore`` owns a ``path_table`` plus one weak slot per row; ``pathlike.store()`` / ``use_store(s)`` select the current store (give an IoC container a ``PathStore`` singleton and ``use_store`` its resolve), ``PathSet(paths, store=s)`` shares the store's table so ``f in ps`` and ``view.to_file()`` need no re-interning. Native text is tokenised straight into the table (warm ``path(s)`` ~2x faster than the raw constructor; first sight ~1.3-2.9x slower — measured in ``benchmarks/path_store.py``), and ``parent`` / ``parents`` / single-component ``/`` derive by row. Design: ``docs/design/pathlike_flyweight.md``.
- PathLike/PathSet (PROTOTYPE, ``pygim.pathlike.PathSet``; ``pygim.pathset`` is untouched): many paths as ONE table — every distinct component interned once (``segment_table``) and every path a hash-consed ``(parent, name)`` row (``path_table.h``), so a path costs ~105-115 bytes all-in (file objects: ~490, PurePath: ~210). Iteration yields ``fileview`` objects that copy nothing and compare/hash like the same path as a ``file``; ``scan()`` reuses one view per pass. ``filter_suffix``/``filter_name``/``filter_absolute`` and ``|``/``&``/``-`` work on rows (~1-10 ns per path on a shared table). ``to_list()`` renders the members (the bridge to DataFrame tools; an Arrow C Data Interface export was prototyped and dropped — the measured reasons are in ``docs/design/pathset_storage.md``). ``PathSet.reserve(n)`` sizes the table up front (automatic for sized iterables; ~14% off a 1M build), rendering walks the chain once into a pre-sized string (~35% faster), and a union across two tables copies the larger table and maps only the other set (~2x faster). Strategies gained ``tokenise()`` (a streaming parse; the POSIX one is allocation-free and proven equal to ``parse_into``). Benchmark: ``benchmarks/pathset_prototype.py``.
