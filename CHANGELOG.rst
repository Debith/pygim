...
Unreleased
----------
- CLI: ``oo docs serve [--dir D] [--port N] [--host H] [--index PAGE] [--rebuild CMD]`` serves a documentation tree locally with a review layer: every served HTML page gets the ✎ commenter (``_commenter.py``; comments POST to ``/comment`` and land one JSON document per line in ``<root>/__notes__/site-comments.jsonl``, read and written by pathlike's JSONL engine; ``/comments?page=`` lists the open ones, ``/comment-edit`` and ``/comment-delete`` change them; a malformed comments file is reported with its line, never rewritten), and an image dropped on a page is written under ``<root>/images/`` (validated segments, image extensions only, never outside the root). ``/`` falls back to the first of ``site/``, ``docs/``, ``build/html/``, ``docs/_build/html/`` with an ``index.html``. Binds all interfaces by default (``$PYGIM_HOST`` or ``--host 127.0.0.1`` for localhost); ``--rebuild`` runs a shell command in the served directory first and aborts on failure. ``oo`` itself is now a group: a verb runs it, free text goes to the assistant, nothing shows the help.
Changed
~~~~~~~
