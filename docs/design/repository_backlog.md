...
| 2.2 | Template pipeline with `if constexpr` dispatch         | done   | `ColumnMajorTranspose::run()` now does one runtime branch (`cpu_supports_avx2()`), then dispatches to `run_impl<Simd::Avx2|Scalar>()` with `if constexpr` and compile-time `static_assert` guard. AVX2 body currently reuses scalar kernel until 2.3 lands. |
| 2.3 | AVX2 8×8 int32 transpose kernel                        | in-progress   | Implemented initial 8×8 AVX2 kernel (`bcp_simd_kernels.h`) and wired it into `ColumnMajorTranspose::run_impl<Simd::Avx2>` for contiguous 4-byte fixed-width 8-column blocks; non-eligible columns and tails use scalar fallback. Dedicated AVX2 TU / per-file compile flags still pending for full cross-platform hardening.                                     |
