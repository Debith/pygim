...
py::str fspath_str(const file& f) { return str_from_text(f.fspath()); }

// Every file handed to Python goes through the current store (path_store.h):
// interned, and the same object as any live one for that value.
py::object wrap(file f) { return make(Engines{}, std::move(f)); }
...
py::object path_factory(py::handle p, py::handle engine) {
    ...
    return make_text(Engines{}, current_store(), t.view, engine_from_arg(engine));
}
...
PYBIND11_MODULE(pathlike, m) {
    ...
    py::class_<file>(m, "file", file_doc().c_str())   // class docs take a bare const char* (py::doc is for functions)
        ...
        .def("__truediv__", [](py::object self, py::handle other) {
                 const text_arg t = text_view_of_arg(other);
                 return joined_with(Engines{}, self, py::cast<const file&>(self), t.view);
             }, py::is_operator())
        .def("__rtruediv__", [](const file& f, py::handle other) { return wrap(f.rjoined(text_from_arg(other))); },
             py::is_operator())
        .def("joinpath",
             [](py::object self, const py::args& parts) {
                 py::object cur = std::move(self);
                 for (const py::handle& part : parts) {
                     const text_arg t = text_view_of_arg(part);
                     cur = joined_with(Engines{}, cur, py::cast<const file&>(cur), t.view);
                 }
                 return cur;
             },
             "Append one or more path components, like pathlib's joinpath().")
        .def_property_readonly("parent", [](py::object self) { return parent_of(Engines{}, self, py::cast<const file&>(self)); },
                               "The parent directory as a file().")
        .def_property_readonly("parents", [](py::object self) { return parents_of(Engines{}, self, py::cast<const file&>(self)); },
    ...
    bind_path_store(Engines{}, m);   // PathStore, store(), use_store() (path_store.h)
    bind_pathset(Engines{}, m);      // PathSet + fileview (pathset.h)
