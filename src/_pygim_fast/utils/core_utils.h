...
[[nodiscard]] inline std::string to_csv(std::vector<std::string> strings, bool sorted = false) {
    if (sorted) {
        std::ranges::sort(strings);
    }
    if (strings.empty()) return {};
    std::size_t total = 0;
    for (const auto& s : strings) total += s.size() + 2;
    std::string result;
...
[[nodiscard]] inline bool is_dunder(std::string_view attr) {
    if (attr.size() < 4) return false;
    return attr[0] == '_' && attr[1] == '_' &&
           attr[attr.size() - 2] == '_' && attr[attr.size() - 1] == '_';
}

[[nodiscard]] inline bool is_dunder(const std::string& attr) {
    return is_dunder(std::string_view{attr});
}

[[nodiscard]] inline bool is_dunder(const char *attr) {
    if (!attr) return false;
    return is_dunder(std::string_view{attr});
}

[[nodiscard]] inline std::string to_lower_copy(std::string value) {
    std::ranges::transform(value, value.begin(), [](unsigned char ch) { return static_cast<char>(std::tolower(ch)); });
    return value;
}

[[nodiscard]] inline double to_bytes(double quantity, const std::string &unit_raw) {
...
[[nodiscard]] inline double to_seconds(double duration, const std::string &unit_raw) {
    const std::string unit = to_lower_copy(unit_raw);
...
[[nodiscard]] inline std::string format_scaled_value(double value,
...
[[nodiscard]] inline std::string calculate_rate(double quantity,
