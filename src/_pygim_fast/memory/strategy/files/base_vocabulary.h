...
dimensions:
  domain:
    ...
    entry:
      ...
      example: A note about spell design is domain=dnd; one about pybind11 is domain=programming.
  ...
  artifact:
    ...
    entry:
      ...
      full: >
        ...
        own: spells and monsters in one, modules and bindings in another.
      ...
      example: A rule about reaction spells is artifact=spell.
  ...
  task:
    ...
    entry:
      ...
      example: A yardstick used while inventing a spell is task=design.
    values:
      design:
        entry:
          ...
          example: Choosing the yardstick for a new defensive reaction.
      critique:
        entry:
          ...
          example: Pointing out that a trigger is ambiguous.
      evaluate:
        entry:
          ...
          example: Checking a spell's damage against the damage table.
      troubleshoot:
        entry:
      ...
      explain:
        entry:
          ...
          example: Why concentration ends when the caster takes damage.
    ...
    values:
      reference:
        entry:
          ...
          example: Shield grants +5 AC until the start of your next turn.
      principle:
        entry:
          ...
          example: A defensive reaction earns its slot only if it beats Shield per slot.
      procedure:
        entry:
          ...
          example: 'Creating a spell: read the space, name it, find its yardstick, set its cost, check balance.'
      example:
        entry:
          brief: One instance.
          when: The text describes a particular case.
          when_not: Not a generalisation drawn from cases — that is a principle.
          example: Frost Ward trades flat AC for typed resistance.
      decision:
        entry:
          ...
          example: Typed resistance was chosen over flat AC for the Ward family.
      preference:
        entry:
          brief: A taste.
          when: The text records how someone likes things done.
          when_not: Not a claim about correctness — that is a principle.
          example: Prefer templates even with one use case.
  ...
  tier:
    ...
    entry:
      ...
      example: A rule about 5th-level characters is tier=mid in dnd.
)yaml";
