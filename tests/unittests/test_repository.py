...
from pygim.repository import acquire_repo, Format, Repository
...
def test_save_load_chain(repo):
    repo.save("table_name", bcp_workers=1)
    repo.load("table_name", load_workers=1)


def test_load_raw_sql_string(repo):
    repo.load("SELECT * FROM foo", load_workers=1)


def test_load_with_query(repo):
