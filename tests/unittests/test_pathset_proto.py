...
set algebra and the Arrow C Data Interface boundary (no libarrow on our side)."""
...
# ── Arrow boundary: our buffers, their consumers ──────────────────────────

def test_paths_column_reaches_pyarrow_and_polars(strs, ps):
    pa = pytest.importorskip("pyarrow")
    col = ps.paths()
    arr = pa.array(col)
    assert arr.type == pa.large_string() and arr.to_pylist() == ps.to_list()
    pl = pytest.importorskip("polars")
    series = pl.from_arrow(arr)
    assert series.to_list() == ps.to_list()
    assert series.str.ends_with(".yaml").sum() == len(ps.filter_suffix(".yaml"))


def test_names_are_a_dictionary_column(ps):
    pa = pytest.importorskip("pyarrow")
    arr = pa.array(ps.names())
    assert pa.types.is_dictionary(arr.type)
    assert arr.to_pylist() == [v.name for v in ps]
    assert len(arr.dictionary) == ps.stats()["segments"] + 1   # the segment table (every component once) plus one blank


def test_from_arrow_round_trip_pyarrow_and_polars(strs, ps):
    pa = pytest.importorskip("pyarrow")
    back = pathlike.PathSet.from_arrow(pa.array(ps.paths()))
    assert back.to_list() == ps.to_list()
    small = pathlike.PathSet.from_arrow(pa.array(["a/b", "c", None, "a//b"]))   # nulls skipped, spellings merged
    assert small.to_list() == [str(P("a/b")), "c"]
    pl = pytest.importorskip("polars")
    s = pl.Series("p", ["x/y.toml", "z"])
    assert pathlike.PathSet.from_arrow(s).to_list() == [str(P("x/y.toml")), "z"]
    assert pathlike.PathSet.from_arrow(pl.DataFrame({"p": ["q/r"]})).to_list() == [str(P("q/r"))]


def test_from_arrow_rejects_non_string_columns():
    pa = pytest.importorskip("pyarrow")
    with pytest.raises(TypeError, match="utf8"):
        pathlike.PathSet.from_arrow(pa.array([1, 2, 3]))
    with pytest.raises(TypeError, match="__arrow_c"):
        pathlike.PathSet.from_arrow(object())
