...
| IoC | `_pygim_fast/wiring/ioc/` | Container keyed by Python interface identity plus optional name. Lifecycle is `transient` or `singleton`; overriding a registration must invalidate cached singleton state. Resolved instances must satisfy `isinstance(instance, interface)` after provider construction and decorator application. Supports opt-in autowiring for class providers via constructor type hints; missing typed dependencies may fall back to Python default values. Keep provider storage, override rules, lifecycle caching, cycle detection, the decorator/validation sequence, and the autowiring *policy* (`plan_autowiring` over neutral `ParamSpec` records; constexpr, static_assert-tested) in core; keep Python key parsing, callability validation, provider/decorator invocation, constructor *introspection* (Python signature → `ParamSpec`), and key-enriched error messages in adapter. Core `resolve()` must work on a descriptor copy: providers may re-enter `register()` and reallocate the registry. |
...
- **Strict dependency policy (STRICT RULE)**: all C++ code compiles on all platforms (Linux/macOS/Windows), locally and in CI — extensions are mandatory and are never skipped or reduced. Anything the build needs (library, header, tool, compiler) must be declared as a dependency and verified to exist *before* compilation begins: `setup.py::_require_dep` (native deps, with per-platform install hints) and `setup.py::_require_compiler` (toolchain). Conditional compilation is permitted only for (1) platform *selection* between complete equivalent implementations, (2) macro hygiene (`NOMINMAX`, `#undef BOOL`, spec-canonical constants the headers omit), (3) explicit opt-in flags whose defaults do not depend on the environment (`PYGIM_BCP_PROFILING`), and (4) the bounded compiler-standard fallback (`_first_supported_std`; code must stay valid at the floor standard). `__has_include` and availability-`#ifdef`s that silently drop functionality are banned and enforced by `test_setup_helpers.py::test_no_availability_conditional_compilation`. Python mirror of the rule: hard dependencies are imported unconditionally — no `except ImportError` fallbacks that change behaviour. Full rationale and incident history: `docs/design/cpp_runtime_linking.md`.
...
- Persistence GIL Pattern: Adapter releases GIL (`py::gil_scoped_release`) before calling core `save()`/`load()`. No `py::object` crosses into `core/`; Arrow C Data Interface is the boundary.
- Persistence BackendPolicy: New backends must satisfy the `BackendPolicy` concept (Connection, SaveImpl, LoadImpl, Dialect, **LoadCache** types + connect/reset/close/name). Verified via `static_assert` in `bindings.cpp`.
- Persistence BCP Ordering: BCP must be enabled on the connection (`SQL_COPT_SS_BCP`) BEFORE `SQLDriverConnect`. Violating this causes silent BCP failures.
- Persistence Connection Pool: `checkout()` returns `std::expected<ConnectionHandle, PoolError>` — timeout and pool-closed are control flow, not exceptions.
- Persistence Arrow Import: `import_record_batch_reader()` must be called WITH GIL held. Supports PyCapsule (`__arrow_c_stream__`), Polars DataFrame (recursive `.to_arrow()` with depth guard), and PyArrow RecordBatchReader/Table.
- Persistence LoadCache: `MssqlLoadCache` provides persistent `LoadConnectionPool` reuse across `load()` calls — eliminates per-load connection establishment cost. `NullLoadCache` is zero-cost for non-MSSQL backends. Cache invalidates on connection string or pool size change.
- Persistence PK Auto-Detect: When `partition_column` is empty and `workers > 1`, `detect_partition_column()` uses SQLPrimaryKeys() + SQLColumns() to find the first integer PK column. Falls back to single-threaded load if none found.
- Persistence Stale Connection Retry: Parallel load retries once on stale connection (SQLSTATE 08S01, 08001, HY000). Clears cache before retry. No proactive health checks (avoids latency on the common path).
